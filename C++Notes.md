...
```c++
try {
    /* program-statements */               // 这里的变量在外部无法访问（具有块作用域）特别是在 catch 子句里也不行
...
假设刚刚执行 `item1` 和 `item2` 加法的代码是与用户交互的代码分离的，其中与用户交互的代码负责处理异常：

```c++
while (cin >> item1 >> item2) {
    try {
       // 执行加法
       // 若失败 代码抛出 runtime_error 异常
}   catch (runtime_error err) {
        cout << err.what() << "\nTry again? Enter y or n" << endl;
        /* err.what() 可以认为是一个字符串 打印"Data must refer to same ID"之类的内容 */
        char c;
        cin >> c;
        if (!cin || c == 'n')   
        break;
